...
\\- Added legacy functions max and min \textit{(i\_funcl.inc)}
...
\end{tabular}
\bigskip
\\You can test this in ISAMPP sandbox game mode with commands \textbf{/lightson} and \textbf{/lightsoff} while being in any vehicle.
\bigskip
...
\begin{sloppypar}
List of all vehicle tires. Can be used with custom functions \textit{MppFixVehicleTires(vehicleid, \textbf{tires})} and \textit{MppPopVehicleTires(vehicleid, \textbf{tires})}. Use bitwise operator OR to create any combination of tires.
\end{sloppypar}
\bigskip
\noindent\begin{tabular}{ |l|l|l| } 
\hline
...
\end{tabular}
\bigskip
\\You can test this in ISAMPP sandbox game mode with commands \textbf{/fixtires} and \textbf{/poptires} while being in any vehicle.
\bigskip
...
List of all available colors for vehicles in game. Colors from carcols.dat file use prefix \textit{CARCOL\_} and color names with prefix \textit{CARCOL\_SAMP\_} are supported in SA-MP version 0.3x or newer.
\bigskip
...
Weather identifiers are included as macro defines with prefix \textit{WEATHER\_}. In ISAMPP sandbox game mode you can test weather settings with command \textbf{/weather [ID]}
\bigskip
