...
\\- New legacy function rot13 \textit{(i\_funcl.inc)}
...
Rotates the alphabet by half of its length - 13 characters. It is a symmetric operation: applying it twice on the same string reveals the original.

\subsection{strrest(const string[], \&index)}
Returns remaining part of the string.

\subsection{strtok(const string[], \&index)}
Strtok is used for splitting strings and was used as one of the methods for creating game commands with arguments.
